...
def test_workspace_dock_assets_are_loaded_with_cachebuster():
    ...
    assert "workspace-dock-themes.css?v=2.7.5-dock-themes-7" in html
    assert "workspace-dock-themes.js?v=2.7.5-dock-themes-7" in html
...
def test_workspace_theme_is_persistent_and_accessible():
    ...
    assert "button.dataset.selectedTheme" in js
    ...
    assert "'midnight-cyan'" in js
    assert "toggle.addEventListener('focus'" not in js
...
def test_desktop_dock_keeps_mobile_drawer_and_hides_badges():
    ...
    assert 'data-workspace-theme="midnight-cyan"' in css
    assert '--workspace-sidebar: #0c1323' in css
