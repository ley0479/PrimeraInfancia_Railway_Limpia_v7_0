...
def register_asistente_capacitacion(app, database_path: str) -> None:
    ...
    def tool_feature_available(tool_name):
        ...
        groups={
            ...
            'admin_enabled':{'get_system_health','get_backup_status','get_module_usage','get_foundation_portfolio','get_liam_center','get_technical_diagnostic'},
            'dev_enabled':{'prepare_dev_change_request','list_dev_change_requests','get_dev_change_review','prepare_dev_sandbox_plan'},
        }
    ...
    def visual_payload(result, module):
        ...
        def table_for(value):
            ...
            if isinstance(value.get('metrics'),list) and isinstance(value.get('priorities'),list) and value.get('role'):
                return {'title':value.get('title'),'note':'Prioridades adaptadas al rol y datos limitados a la sesión activa.','metrics':value['metrics'],'columns':['Prioridad del rol','Orden'],'rows':[[label,index] for index,label in enumerate(value['priorities'],1)]}
            if isinstance(value.get('agenda'),list) and value.get('draft_only') is True:
                return {'title':value.get('title'),'note':'Borrador previo de solo lectura. Las tareas posteriores requieren aprobación.','metrics':[{'label':'Asuntos','value':len(value['agenda'])},{'label':'Fuentes disponibles','value':len(value.get('sources') or [])},{'label':'Periodo','value':value.get('period') or 'Actual'}],'columns':['Prioridad','Asunto','Valor','Fuente'],'rows':[[x.get('priority'),x.get('topic'),x.get('value'),x.get('source')] for x in value['agenda']]}
            if isinstance(value.get('commitments'),list) and value.get('tasks_created') is False:
                s=value.get('summary') or {};return {'title':value.get('title'),'note':'Borrador de seguimiento. No se crearon tareas.','metrics':[{'label':'Compromisos','value':s.get('total',0)},{'label':'Completos','value':s.get('complete',0)},{'label':'Por completar','value':s.get('incomplete',0)}],'columns':['Compromiso','Responsable','Fecha','Estado','Datos faltantes'],'rows':[[x.get('title'),x.get('responsible'),x.get('due_date'),x.get('status'),', '.join(x.get('missing') or [])] for x in value['commitments']]}
            if isinstance(value.get('request'),dict) and value.get('code_modified') is False:
                item=value['request'];return {'title':'Solicitud técnica registrada','note':'Borrador ADMIN/DEV. No se modificó código ni se inició despliegue.','metrics':[{'label':'Solicitud','value':item.get('request_id')},{'label':'Estado','value':item.get('status')},{'label':'Módulo','value':item.get('module')}],'columns':['Etapa','Orden'],'rows':[[step,index] for index,step in enumerate(value.get('workflow') or [],1)],'sections':[{'title':'Objetivo','text':item.get('objective')},{'title':'Impacto','text':item.get('impact_summary')}]}
            if isinstance(value.get('requests'),list) and value.get('source')=='Centro Liam ADMIN/DEV':return {'title':'Solicitudes técnicas','metrics':[{'label':'Borradores','value':value.get('total',0)}],'columns':['Solicitud','Módulo','Objetivo','Estado','Fecha'],'rows':[[x.get('request_id'),x.get('module'),x.get('objective'),x.get('status'),x.get('created_at')] for x in value['requests']]}
            if isinstance(value.get('module_review'),dict) and isinstance(value.get('gates'),list):
                item=value.get('request') or {};review=value['module_review'];return {'title':f"Revisión técnica {item.get('request_id')}",'note':'Paquete verificable de solo lectura. No modifica código ni autoriza despliegues.','metrics':[{'label':'Estado','value':item.get('status')},{'label':'Módulo','value':review.get('title')},{'label':'Controles registrados','value':'Sí' if review.get('registered_controls') else 'No'}],'columns':['Compuerta','Estado','Requisito'],'rows':[[x.get('gate'),x.get('status'),x.get('requirement')] for x in value['gates']],'sections':[{'title':'Objetivo','text':item.get('objective')},{'title':'Fuente del módulo','text':review.get('data_source')},{'title':'Uso posterior','text':review.get('downstream_use')}]}
            ...
            if value.get('scope',{}).get('type')=='authorized_system_backup' and value.get('sanitized') is True:
                latest=value.get('latest') or {};summary=value.get('summary') or {}
                return {'title':'Copias de seguridad','metrics':[{'label':'Total','value':summary.get('total',0)},{'label':'Válidas','value':summary.get('valid',0)},{'label':'Con error','value':summary.get('errors',0)},{'label':'Última copia','value':latest.get('fecha_creacion') or 'Sin registros'}],'columns':['Motivo','Estado','Integridad','Tamaño','Creación','Validación'],'rows':[[latest.get('motivo'),latest.get('estado'),latest.get('integridad'),latest.get('tamano_bytes'),latest.get('fecha_creacion'),latest.get('fecha_validacion')]] if latest else []}
            if isinstance(value.get('items'),list) and value.get('source')=='Auditoría funcional de LIAM':
                s=value.get('summary') or {};return {'title':f"Uso de módulos · {value.get('period_days',30)} días",'note':'La clasificación es descriptiva. Liam no elimina ni desactiva módulos.','metrics':[{'label':'Eventos','value':s.get('events',0)},{'label':'Frecuentes','value':s.get('frequent',0)},{'label':'Poco usados','value':s.get('low_usage',0)},{'label':'Sin actividad','value':s.get('without_activity',0)}],'columns':['Módulo','Usos','Usuarios','Último uso','Clasificación'],'rows':[[x.get('module'),x.get('uses'),x.get('users'),x.get('last_used'),x.get('category')] for x in value['items']]}
            ...
            if isinstance(value.get('known_solution'),dict):
                item=value['known_solution'];return {'title':f"Solución conocida · {item.get('code')}",'note':'Orientación sanitizada de la Base de Conocimiento LIAM; no incluye mensajes técnicos originales.','metrics':[{'label':'Ocurrencias','value':item.get('occurrences',0)},{'label':'Resueltas','value':item.get('resolved_occurrences',0)},{'label':'Estado reciente','value':item.get('latest_status') or 'Sin incidente local'}],'sections':[{'title':'Causa orientativa','text':item.get('cause')},{'title':'Solución recomendada','text':item.get('solution')}],'columns':[],'rows':[]}
            if isinstance(value.get('diagnostic'),dict) and value.get('source')=='Centro de Incidencias LIAM':
                item=value['diagnostic'];return {'title':f"Diagnóstico técnico · {item.get('incident_id')}",'note':'Vista elevada sanitizada. No contiene secretos ni datos de otras fundaciones.','metrics':[{'label':'Código','value':item.get('error_code')},{'label':'HTTP','value':item.get('http_status')},{'label':'Estado','value':item.get('status')},{'label':'Severidad','value':item.get('severity')}],'sections':[{'title':'Módulo y operación','text':f"{item.get('module') or 'Sin módulo'} · {item.get('action') or 'Sin operación'}"},{'title':'Excepción sanitizada','text':item.get('exception_sanitized') or 'Sin mensaje técnico disponible.'},{'title':'Traza','text':item.get('trace_id') or 'Sin traza disponible.'}],'columns':[],'rows':[]}
            if isinstance(value.get('artifact'),dict) and value.get('requires_external_isolated_runner') is True:
                item=value['artifact'];architecture=item.get('architecture') or {};risk=item.get('risk') or {};files=architecture.get('files') or [];tests=item.get('tests') or []
                return {'title':f"Plan de sandbox · {value.get('request_id')}",'note':'Este plan no modificó código ni ejecutó comandos. Requiere un runner aislado externo.','metrics':[{'label':'Estado','value':item.get('status')},{'label':'Riesgo','value':risk.get('level')},{'label':'Archivos permitidos','value':len(files)},{'label':'Pruebas planificadas','value':len(tests)}],'columns':['Tipo','Ruta'],'rows':[['Archivo permitido',path] for path in files]+[['Prueba',path] for path in tests]}
        ...
        if proposal:
            ...
            if target_module and allowed and target_module not in allowed:
                ...
                try:
                    ...
                    if proposal['server_tool']=='get_foundation_data_summary':
                    ...
                    elif proposal['server_tool']=='get_role_dashboard':
                        values={item.get('label'):item.get('value') for item in tool_result.get('metrics') or []};message=f"Tu tablero de {tool_result.get('role','usuario').title()}: {values.get('Tareas pendientes',0)} tareas pendientes, {values.get('Tareas vencidas',0)} vencidas, {values.get('Entregables pendientes',0)} entregables pendientes y {values.get('Alertas críticas',0)} alertas críticas. Los datos corresponden exclusivamente a tu alcance autorizado."
                        actions=[]
                    elif proposal['server_tool']=='prepare_meeting_brief':
                        critical=sum(1 for item in tool_result.get('agenda') or [] if str(item.get('priority')).upper() in {'CRITICO','ALTA','ALTO'});message=f"Preparé el resumen previo de reunión con {len(tool_result.get('agenda') or [])} asuntos y {critical} puntos prioritarios. Consulté {len(tool_result.get('sources') or [])} fuentes disponibles. No creé tareas ni compromisos automáticamente."
                        actions=[]
                    elif proposal['server_tool']=='prepare_meeting_followup':
                        summary=tool_result.get('summary') or {};message=f"Preparé {summary.get('total',0)} compromisos como borradores: {summary.get('complete',0)} completos y {summary.get('incomplete',0)} por completar. No creé tareas; cualquier creación requerirá confirmación."
                        actions=[]
                    elif proposal['server_tool']=='prepare_dev_change_request':
                        item=tool_result.get('request') or {};message=f"Registré la solicitud técnica {item.get('request_id')} para {item.get('module')} en estado borrador. No modifiqué código, no ejecuté pruebas y no inicié despliegue."
                        actions=[]
                    elif proposal['server_tool']=='list_dev_change_requests':
                        message=f"Encontré {tool_result.get('total',0)} solicitudes técnicas tuyas dentro de la fundación activa."
                        actions=[]
                    elif proposal['server_tool']=='get_dev_change_review':
                        item=tool_result.get('request') or {};review=tool_result.get('module_review') or {};pending=sum(1 for gate in tool_result.get('gates') or [] if gate.get('status') in {'PENDING','BLOCKED'});message=f"Preparé la revisión de {item.get('request_id')} para {review.get('title')}: {pending} compuertas todavía requieren trabajo o aprobación. No inferí archivos, no modifiqué código y no inicié despliegue."
                        actions=[]
                    elif proposal['server_tool']=='prepare_dev_sandbox_plan':
                        item=tool_result.get('artifact') or {};message=f"Preparé el plan de sandbox {item.get('artifact_id')} para {tool_result.get('request_id')}: {len((item.get('architecture') or {}).get('files') or [])} archivos permitidos y {len(item.get('tests') or [])} pruebas planificadas. No modifiqué código, no ejecuté comandos y no inicié despliegue."
                        actions=[]
                        ...
                        actions=[]
                    elif proposal['server_tool']=='get_backup_status':
                        latest=tool_result.get('latest') or {};summary=tool_result.get('summary') or {};message=f"Copias de seguridad: {summary.get('total',0)} registradas, {summary.get('valid',0)} válidas y {summary.get('errors',0)} con error. La última fue creada el {latest.get('fecha_creacion') or 'sin registro disponible'} con estado {latest.get('estado') or 'no disponible'}. No consulté rutas ni hashes."
                        actions=[]
                    elif proposal['server_tool']=='get_module_usage':
                        summary=tool_result.get('summary') or {};message=f"Uso de módulos en los últimos {tool_result.get('period_days',30)} días: {summary.get('events',0)} eventos, {summary.get('frequent',0)} frecuentes, {summary.get('low_usage',0)} poco usados y {summary.get('without_activity',0)} sin actividad. No se eliminó ni desactivó ningún módulo."
                        actions=[]
                        ...
                        actions=[]
                    elif proposal['server_tool']=='get_known_solution':
                        item=tool_result.get('known_solution') or {};message=f"Para {item.get('code')} encontré {item.get('occurrences',0)} ocurrencias dentro de tu fundación, de las cuales {item.get('resolved_occurrences',0)} están resueltas. Causa orientativa: {item.get('cause')} Solución recomendada: {item.get('solution')}"
                        actions=[]
                    elif proposal['server_tool']=='get_technical_diagnostic':
                        item=tool_result.get('diagnostic') or {};message=f"Diagnóstico técnico sanitizado de {item.get('incident_id')}: módulo {item.get('module') or 'sin módulo'}, operación {item.get('action') or 'sin operación'}, HTTP {item.get('http_status')}, código {item.get('error_code')} y estado {item.get('status')}. Traza: {item.get('trace_id') or 'no disponible'}."
                        actions=[]
    ...
    def realtime_voice_call():
        ...
        realtime_tools=[
            ...
            {'type':'function','name':'get_role_dashboard','description':'Construye el tablero conversacional del usuario con tareas, entregables, alertas y prioridades adaptadas a su rol y alcance.','parameters':{'type':'object','properties':{'period':{'type':'string'},'scope':{'type':'string','enum':['self','team']}},'additionalProperties':False}},
            {'type':'function','name':'prepare_meeting_brief','description':'Prepara un resumen previo de reunión con indicadores, pendientes y alertas del alcance autorizado. No crea tareas.','parameters':{'type':'object','properties':{'period':{'type':'string'}},'additionalProperties':False}},
            {'type':'function','name':'prepare_meeting_followup','description':'Estructura compromisos posteriores como borradores. No crea tareas sin confirmación.','parameters':{'type':'object','properties':{'commitments':{'type':'array','maxItems':20,'items':{'type':'object','properties':{'title':{'type':'string'},'responsible':{'type':'string'},'due_date':{'type':'string'}},'required':['title','responsible','due_date'],'additionalProperties':False}}},'required':['commitments'],'additionalProperties':False}},
            {'type':'function','name':'prepare_dev_change_request','description':'Exclusivo de SUPERADMIN. Registra una solicitud técnica como borrador; no modifica código ni despliega.','parameters':{'type':'object','properties':{'module':{'type':'string'},'objective':{'type':'string'}},'required':['module','objective'],'additionalProperties':False}},
            {'type':'function','name':'list_dev_change_requests','description':'Exclusivo de SUPERADMIN. Lista los borradores técnicos propios dentro de la fundación activa.','parameters':{'type':'object','properties':{'limit':{'type':'integer','minimum':1,'maximum':100}},'additionalProperties':False}},
            {'type':'function','name':'get_dev_change_review','description':'Exclusivo de SUPERADMIN. Prepara una revisión verificable y de solo lectura de una solicitud técnica propia.','parameters':{'type':'object','properties':{'request_id':{'type':'string'}},'required':['request_id'],'additionalProperties':False}},
            {'type':'function','name':'prepare_dev_sandbox_plan','description':'Exclusivo de SUPERADMIN. Genera un plan cerrado de archivos, riesgos y pruebas para un sandbox externo; no ejecuta código.','parameters':{'type':'object','properties':{'request_id':{'type':'string'}},'required':['request_id'],'additionalProperties':False}},
            ...
            {'type':'function','name':'get_backup_status','description':'Exclusivo de SUPERADMIN. Consulta estado, fecha, tipo y tamaño del último backup sin exponer rutas, archivos ni hashes.','parameters':{'type':'object','properties':{},'additionalProperties':False}},
            {'type':'function','name':'get_module_usage','description':'Consulta actividad de módulos de la fundación activa y clasifica frecuentes, poco usados o sin actividad. Nunca elimina módulos.','parameters':{'type':'object','properties':{'days':{'type':'integer','minimum':1,'maximum':365}},'additionalProperties':False}},
            ...
            {'type':'function','name':'get_known_solution','description':'Busca por código una solución sanitizada del catálogo o de incidencias de la fundación activa. No expone mensajes técnicos originales.','parameters':{'type':'object','properties':{'code':{'type':'string'}},'required':['code'],'additionalProperties':False}},
            {'type':'function','name':'get_technical_diagnostic','description':'Exclusivo de SUPERADMIN. Consulta el diagnóstico técnico sanitizado de una incidencia exacta dentro de la fundación activa.','parameters':{'type':'object','properties':{'incident_id':{'type':'string'}},'required':['incident_id'],'additionalProperties':False}},
