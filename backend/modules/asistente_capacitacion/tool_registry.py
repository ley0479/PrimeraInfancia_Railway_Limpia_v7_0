...
def _dev_change_review(database_path:str,tenant_id:int,args:dict,user:dict)->dict:
    ...
    return {'scope':{'foundation_id':tenant_id,'source':'authenticated_session','cross_foundation':False},'request':request_item,'module_review':review,'gates':gates,'sandbox_plan':dict(artifact) if artifact else None,'sandbox_result':result_summary,'facts_verified_from_registry':True,'files_inferred':False,'code_modified':False,'sandbox_diff_registered':bool(result_summary),'tests_executed':bool(result_summary),'deployment_started':False,'source':'Centro Liam ADMIN/DEV','read_only':True}
