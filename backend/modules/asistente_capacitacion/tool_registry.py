...
def _prepare_dev_sandbox_plan(database_path:str,tenant_id:int,args:dict,user:dict)->dict:
    ...
    try:
        ...
        request_item=dict(row);scope=dev_plan_for(request_item['module']);generated_test=f"backend/tests/generated/test_{request_id.lower().replace('-','_')}.py";content={'request_id':request_id,'objective':request_item['objective'],'architecture':{'module':scope['module'],'files':scope['allowed_files']},'risk':{'level':'HIGH' if scope['module'] in {'administracion','backups','facturacion'} else 'MEDIUM','tenant_review_required':True,'permission_review_required':True},'tests':scope['test_files'],'generated_test_path':generated_test,'sandbox':{'isolated_workspace_required':True,'production_writes':False,'network_default':'disabled','commands_generated':False},'forbidden_paths':scope['forbidden_paths'],'rollback':'Revertir únicamente el commit aprobado de la solicitud.'}
...
def _dev_change_review(database_path:str,tenant_id:int,args:dict,user:dict)->dict:
    ...
    try:
        ...
        result_artifact=conn.execute("SELECT artifact_id,content_json,content_sha256,status FROM lia_dev_change_artifacts WHERE request_id=? AND fundacion_id=? AND usuario_id=? AND artifact_type='SANDBOX_RESULT'",(request_id,tenant_id,uid)).fetchone()
        approval_artifact=conn.execute("SELECT artifact_id,content_sha256,status FROM lia_dev_change_artifacts WHERE request_id=? AND fundacion_id=? AND usuario_id=? AND artifact_type='APPROVAL'",(request_id,tenant_id,uid)).fetchone()
        rollback_artifact=conn.execute("SELECT artifact_id,content_json,content_sha256,status FROM lia_dev_change_artifacts WHERE request_id=? AND fundacion_id=? AND usuario_id=? AND artifact_type='ROLLBACK_PLAN'",(request_id,tenant_id,uid)).fetchone()
    finally:conn.close()
    if not row:raise LookupError('No encontré esa solicitud dentro de tu usuario y fundación activa.')
    ...
    planned=bool(artifact);result_status=str(result_artifact['status']) if result_artifact else '';passed=result_status=='PASSED';failed=result_status=='FAILED';approved=bool(approval_artifact) and str(approval_artifact['status'])=='APPROVED';gates=[{'gate':'Arquitectura','status':'READY' if planned else 'PENDING','requirement':'Identificar servicios, endpoints, tablas, frontend, permisos y tenant afectados.'},{'gate':'Riesgo','status':'READY' if planned else 'PENDING','requirement':'Clasificar impacto, compatibilidad, seguridad y rollback.'},{'gate':'Pruebas','status':'READY' if passed else ('FAILED' if failed else ('PLANNED' if planned else 'PENDING')),'requirement':'Definir y ejecutar pruebas unitarias, integración, permisos, tenant y regresión.'},{'gate':'Diff','status':'READY' if passed else ('FAILED' if failed else 'PENDING'),'requirement':'Preparar cambios pequeños y mostrar el diff antes de aprobación.'},{'gate':'Aprobación','status':'READY' if approved else ('PENDING' if passed else 'BLOCKED'),'requirement':'Requiere aprobación explícita y reautenticada después de pruebas satisfactorias.'},{'gate':'Despliegue','status':'DISABLED','requirement':'No se despliega automáticamente a producción.'}]
    result_summary=None
    if result_artifact:
        content=json.loads(result_artifact['content_json']);result_summary={'artifact_id':result_artifact['artifact_id'],'content_sha256':result_artifact['content_sha256'],'status':result_status,'runner':content.get('runner'),'changed_paths':content.get('changed_paths') or [],'git_diff_check':content.get('git_diff_check'),'tests':content.get('tests') or [],'diff_preview':str(content.get('diff') or '')[:12000],'deployment_started':False}
    rollback_summary=None
    if rollback_artifact:
        content=json.loads(rollback_artifact['content_json']);rollback_summary={'artifact_id':rollback_artifact['artifact_id'],'content_sha256':rollback_artifact['content_sha256'],'status':rollback_artifact['status'],'base_commit':content.get('base_commit'),'changed_paths':content.get('changed_paths') or [],'strategy':content.get('strategy'),'automatic_execution':False}
    return {'scope':{'foundation_id':tenant_id,'source':'authenticated_session','cross_foundation':False},'request':request_item,'module_review':review,'gates':gates,'sandbox_plan':dict(artifact) if artifact else None,'sandbox_result':result_summary,'approval':dict(approval_artifact) if approval_artifact else None,'rollback_plan':rollback_summary,'facts_verified_from_registry':True,'files_inferred':False,'code_modified':False,'sandbox_diff_registered':bool(result_summary),'tests_executed':bool(result_summary),'deployment_started':False,'source':'Centro Liam ADMIN/DEV','read_only':True}
