(function(){
  ...
  function waitForIceGathering(pc,timeoutMs=8000){
    ...
    return new Promise(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;clearTimeout(timer);pc.removeEventListener('icegatheringstatechange',check);resolve()};const check=()=>{if(pc.iceGatheringState==='complete')finish()};const timer=setTimeout(finish,timeoutMs);pc.addEventListener('icegatheringstatechange',check)});
  }
  function friendlyError(error){
    const name=String(error?.name||''),message=String(error?.message||'');
    if(['NotAllowedError','PermissionDeniedError'].includes(name))return new Error('El navegador no tiene permiso para usar el micrófono. Habilítalo en el candado de la barra de direcciones e intenta nuevamente.');
    if(['NotFoundError','DevicesNotFoundError'].includes(name))return new Error('No se encontró un micrófono disponible para iniciar la conversación.');
    if(name==='NotReadableError')return new Error('El micrófono está siendo usado por otra aplicación. Ciérrala e intenta nuevamente.');
    if(message)return error;
    return new Error('No fue posible iniciar la conversación en tiempo real.');
  }
  ...
  async function start({endpoint,headers={},module='dashboard'}={}){
    ...
    try{
      ...
      pc.onconnectionstatechange=()=>{if(['failed','disconnected','closed'].includes(pc.connectionState)&&(state.active||state.connecting))stop(`connection_${pc.connectionState}`)};
      ...
      const response=await fetch(endpoint,{method:'POST',headers:{...headers,'Content-Type':'application/sdp','X-Liam-Module':module},body:localSdp});
    ...
    }catch(error){const normalized=friendlyError(error);cleanup();emit('error',{message:normalized.message});throw normalized}
  }
