(function(){
  ...
  function create(){if(active?.isConnected)return active;remove();const wrap=document.getElementById('liam-avatar-wrap');if(!wrap)return null;const source=wrap.querySelector('.ian-avatar-2d, .ian-avatar-visual, .ian-avatar-svg')?.cloneNode(true);if(!source)return null;active=document.createElement('div');active.id='ian-tour-avatar';active.className='ian-tour-avatar';active.setAttribute('aria-hidden','true');active.appendChild(source);document.body.appendChild(active);wrap.classList.add('ian-panel-avatar-away');return active}
  ...
  async function moveToControl(helpId,options={}){
    ...
    const avatarRect=avatar.getBoundingClientRect(),targetCenter=target.left+target.width/2,avatarCenter=avatarRect.left+avatarRect.width/2;state(targetCenter<avatarCenter?'pointing_left':'pointing_right');window.LIAM_ANIMATION?.highlight(helpId,options.message||'Herramienta señalada por LIAM');return true;
  }
