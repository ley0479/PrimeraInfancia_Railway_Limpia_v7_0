# Matriz de cobertura de ELIAN

La autorización real se obtiene del backend; una ruta no autorizada nunca se incorpora al recorrido del usuario.

| Orden | Módulo | Ruta segura | Guía | Recorrido completo | Controles | Estado |
|---:|---|---|---|---|---|---|
| 10 | Centro de Control | `dashboard` | Sí | Sí | Sí | Implementado |
| 20 | Base Maestra y Cuéntame | `base-maestra` | Sí | Sí | Sí | Implementado |
| 30 | Talento Humano | `talento` | Sí | Sí | Sí | Implementado |
| 40 | Salud y Nutrición | `salud-nutricion` | Sí | Sí | Sí | Implementado |
| 50 | Calendario | `calendario-inteligente` | Sí | Sí | Sí | Implementado |
| 60 | Motor Documental | `motor-documental` | Sí | Sí | Sí | Implementado |
| 70 | Planeación Pedagógica | `planeacion-pedagogica` | Sí | Sí | Sí | Implementado |
| 80 | Gestión Pedagógica | `gestion-pedagogica` | Sí | Sí | Sí | Implementado |
| 90 | Gestión Psicosocial | `componente-psicosocial` | Sí | Sí | Sí | Implementado |
| 100 | Familias y Redes | `familias-redes` | Sí | Sí | Sí | Implementado |
| 110 | Formatos | `formatos` | Sí | Sí | Sí | Implementado |
| 120 | Reportes | `reportes-gerenciales` | Sí | Sí | Sí | Implementado |
| 130 | Relación del Mes | `relacion-mes` | Sí | Sí | Sí | Implementado |
| 140 | Expediente UCA | `expediente-operativo-uca` | Sí | Sí | Sí | Implementado |
| 150 | Administración | `administracion` | Sí | Sí | Sí | Implementado |
| 160 | Créditos y Licencias | `facturacion` | Sí | Sí | Sí | Implementado |
| 170 | Auditoría e Integridad | `integrity-stability` | Sí | Sí | Sí | Implementado |
| 180 | Ayuda y Soporte | `manual-operativo` | Sí | Sí | Sí | Implementado |

Todos tienen explicación estructurada de propósito, usuarios autorizados, entradas, fuente, validaciones, resultados, consumidores, errores y siguiente paso. Los controles principales del recorrido están registrados para señalamiento seguro.

## Recursos visuales

- Afrocolombiano institucional masculino: disponible.
- Afrocolombiana institucional femenina: disponible.
- Variante tecnológica masculina y femenina: disponible.
- Variante educativa masculina y femenina: disponible.
